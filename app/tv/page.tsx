...
const TV_KIOSK_ID = "kiosk-TV";
const POLL_INTERVAL_MS = 30_000;

type TvMediaRow = Omit<MediaItem, "duration" | "sort_order"> & {
  created_at?: string;
...
export default function TvPage() {
  ...
  const fetchMedia = useCallback(async () => {
    ...
    if (error) {
      console.error("Error fetching TV media:", error);
      setLoadError(true);
      setLoading(false);
      return;
    }

    const normalized = ((data ?? []) as TvMediaRow[])
      .map((item, index) => ({
        ...item,
        duration: item.duration ?? 10,
        row_slot: 1 as const,
        kiosk_id: TV_KIOSK_ID,
  ...
  if (loading && mediaList.length === 0) {
    ...
    displayContent = (
      <MediaSlideshow
        items={mediaList}
        emptyMessage="ยังไม่มีสื่อสำหรับ TV"
      />
    );
  }
